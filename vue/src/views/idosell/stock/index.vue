<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="128px">
      <el-form-item :label="$t('product.id')" prop="productId">
        <el-input
...
<!--      <el-table-column label="主键ID" align="center" prop="id" />-->
      <el-table-column :label="$t('product.id')" align="center" prop="productId" />
...
<!--      <el-table-column :label="$t('product.skuId')" align="center" prop="skuId" />-->
      ...
      <el-table-column :label="$t('product.size')" align="center" prop="sizeId" />
...
import {pullShopStock,syncGoodsInventoryToOms,listShopStock} from "@/api/idosell/stock";
  ...
  methods: {
    ...
    handlePushStockToOms(row) {
      this.$modal.confirm('是否确认同步该商品sku库存到OMS？').then(() => {
        ...
        syncGoodsInventoryToOms({id:row.id}).then(resp=>{
