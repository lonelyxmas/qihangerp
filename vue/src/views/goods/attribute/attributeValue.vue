...
<script>
import { listAttributeValue } from "@/api/goods/attribute";
...
export default {
  ...
  created() {
    if(this.$route.query.attributeId){
      this.queryParams.attributeId = this.$route.query.attributeId
      this.queryParams.attributeName = this.$route.query.attributeName
    }
    this.getList();
  },
  methods: {
    ...
    getList() {
      this.loading = true;
      listAttributeValue(this.queryParams).then(response => {
